import UserService from '../../../src/document-based/services/user_service.js';
...
describe('UserService Tests', () => {
    ...
    test('UserService.create valid partitions', async () => {
    ...
    test.each([
        ...
        [ { body: { uuid: user.uuid, email: 'test@test.test', username: 'test', password: 'test' } }, 'UUID must be unique' ],
        [ { body: { uuid: 'test', email: user.email, username: 'test', password: 'test' } }, 'Email must be unique' ],
        [ { body: { uuid: 'test', email: 'test@test.test', username: user.username, password: 'test' } }, 'Username must be unique' ],
    ])('UserService.create invalid partitions', async (options, expected) => {
        expect(() => UserService.create(options)).rejects.toThrowError(expected);
    });
    ...
    test.each([
        ...
        [{ body: context.member }],
        [{ body: user }],
    ])('UserService.login valid partitions', async (options) => {
    ...
    ])('UserService.login invalid partitions', async (options, expected) => {
        expect(() => UserService.login(options)).rejects.toThrowError(expected);
    });

    test.each([
        [{ uuid: context.admin.sub }],
        [{ uuid: context.mod.sub }],
        [{ uuid: context.member.sub }],
        [{ uuid: user.uuid }],
    ])('UserService.findOne valid partitions', async ({ uuid }) => {
    ...
    test('UserService.update valid partitions', async () => {
    ...
    ])('UserService.update invalid partitions', async (options, expected) => {
        expect(() => UserService.update(options)).rejects.toThrowError(expected);
    });

    test('UserService.createUserLogin valid partitions', async () => {
        const result = await UserService.createUserLogin({ uuid: user.uuid, body: {
            uuid: login_uuid,
    ...
    ])('UserService.createUserLogin invalid partitions', async (options, expected) => {
        expect(() => UserService.createUserLogin(options)).rejects.toThrowError(expected);
    });
    ...
    test.each([
        ...
        [{ uuid: user.uuid }, 2],
    ])('UserService.getUserLogins valid partitions', async (options, length) => {
    ...
    ])('UserService.getUserLogins invalid partitions', async (options, expected) => {
        expect(() => UserService.getUserLogins(options)).rejects.toThrowError(expected);
    });

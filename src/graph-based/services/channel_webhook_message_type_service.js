...
class Service {
    async findOne(options = { name: null }) {
        ...
        const result = await neodeInstance.model('ChannelWebhookMessageType').find(options.name);
    ...
    async findAll(options = { page: null, limit: null }) {
        ...
        const result = await neodeInstance.batch([
            { query:
                `MATCH (uss:ChannelWebhookMessageType) ORDER BY uss.created_at DESC ` +
            ...
            { query: `MATCH (uss:ChannelWebhookMessageType) RETURN COUNT(uss) AS count`, params: {} },
        ]);
